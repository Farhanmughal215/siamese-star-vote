  ...
  return (
    <>
      {justSignedUp && (
        <p
          role="status"
          className="mb-3 rounded-2xl border border-mint-dark/40 bg-mint-light/50 px-3.5 py-2.5 text-[13px] font-medium text-brown"
        >
          🎉 Account created! Sign in below to start hearting cats.
        </p>
      )}
      <form action={formAction} className="space-y-3.5">
        ...
        {state.error && (
          <p
            role="alert"
            className="rounded-2xl border border-pink-dark/30 bg-pink-light/30 px-3.5 py-2 text-[13px] font-medium text-brown"
          >
            {state.error}
          </p>
        )}
    ...
    <main className="relative flex min-h-screen items-center justify-center px-4 py-10">
      ...
      <div className="relative w-full max-w-md">
        ...
        >
          ...
          <Suspense
            fallback={
              <div className="mt-5 h-44 animate-pulse rounded-2xl bg-white/40" />
            }
          >
            <SignInForm />
          </Suspense>
